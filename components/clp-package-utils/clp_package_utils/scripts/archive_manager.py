...
def main(argv: List[str]) -> int:
    ...
    storage_type: StorageType = StorageType(clp_config.archive_output.storage.type)
    if StorageType.FS != storage_type:
        logger.error(f"Archive manager is not supported for storage type: {storage_type}.")
        return -1

    storage_engine: StorageEngine = StorageEngine(clp_config.package.storage_engine)
