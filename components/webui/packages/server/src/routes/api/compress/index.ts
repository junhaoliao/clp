...
const plugin: FastifyPluginAsyncTypebox = async (fastify) => {
    ...
    fastify.post(
        ...
        {
            schema: {
                body: CompressionJobCreationSchema as never,
                response: {
        ...
        async (request, reply) => {
            const {
                ...
                timestampKey,
                unstructured,
            } = request.body as CompressionJobCreation;
            ...
            (jobConfig.input as ClpIoFsInputConfig).paths_to_compress = paths.map(
                (path: string) => join(settings.LogsInputRootDir, path)
            );
