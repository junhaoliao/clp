...
import {clpAggregationToDataFrame, clpDocumentsToDataFrame} from "../clp";
...
describe("clpDocumentsToDataFrame", () => {
  ...
  it("handles missing optional fields gracefully", () => {
    ...
    const ds = frame.fields.find((f) => f.name === "dataset");
    const aid = frame.fields.find((f) => f.name === "archive_id");
...
describe("clpAggregationToDataFrame", () => {
  ...
  it("converts timeline bucket documents", () => {
    ...
    expect(ts!.name).toBe("timestamp");
    expect(ts!.type).toBe("time");
    expect(ts!.values).toEqual([1715000000000, 1715000060000, 1715000120000]);

    expect(cnt!.name).toBe("count");
    expect(cnt!.type).toBe("number");
    expect(cnt!.values).toEqual([5, 12, 3]);
  });
});
