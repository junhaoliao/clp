...
const buildS3JobConfig = (
    ...
    const authType = settings.LogsInputS3AwsAuthType;
    if (null === authType) {
    ...
    const input: ClpIoS3InputConfig = {
        ...
        aws_authentication: {
            type: authType as AwsAuthType,
            profile: settings.LogsInputS3AwsProfile,
            credentials: (null !== awsAccessKeyId && null !== awsSecretAccessKey) ?
                {
...
const plugin: FastifyPluginAsyncTypebox = async (fastify) => {
    ...
    fastify.post(
        ...
        async (request, reply) => {
            try {
                const jobConfig: ClpIoConfig = (CompressionJobInputType.S3 === request.body.inputType) ?
                    buildS3JobConfig(
                        request.body,
                        request.log,
