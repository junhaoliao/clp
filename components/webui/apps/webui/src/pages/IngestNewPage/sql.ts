...
function buildArchivesFrom (datasetNames: string[]): string {
    ...
    const parts = datasetNames.map((name) => `SELECT ${CLP_ARCHIVES_TABLE_COLUMN_NAMES.UNCOMPRESSED_SIZE}, ` +
...
function buildFilesFrom (datasetNames: string[]): string {
    ...
    const parts = datasetNames.map((name) => `SELECT ${CLP_FILES_TABLE_COLUMN_NAMES.ORIG_FILE_ID}, ` +
