...
const validateDatasetName = (datasetName: string): string | null => {
    ...
    if (false === Value.Check(DatasetNameSchema as unknown as import("@sinclair/typebox").TSchema, datasetName)) {
        const [firstError] = [...Value.Errors(DatasetNameSchema as unknown as import("@sinclair/typebox").TSchema, datasetName)];
