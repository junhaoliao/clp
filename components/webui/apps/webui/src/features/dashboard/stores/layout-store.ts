    ...
    temporal(
        (set, get) => ({
            ...
            setDashboard: (dashboard) => {
                set({dashboard, isLoading: false, error: null, isDirty: false, activeTabId: dashboard.tabs?.[0]?.id ?? null});
            },
            updatePanel: (panelId, updates) => {
                ...
                set({
                    dashboard: {
                        ...d,
                        panels: d.panels.map((p) => (p.id === panelId ?
                            {...p, ...updates} :
                            p)),
                    },
                    isDirty: true,
            ...
            addPanel: (type) => {
                ...
                const newPanel: DashboardPanel = {
                    ...
                    queries: requiresQuery ?
                        [{refId: "A", datasource: {type: dsType, uid: "default"}, query}] :
                        [],
                    options: {},
                };
            ...
            removePanel: (panelId) => {
                ...
                set({
                    ...
                    isDirty: true,
                    selectedPanelId: get().selectedPanelId === panelId ?
                        null :
                        get().selectedPanelId,
                });
            },
            ...
            addTab: (title) => {
                ...
                const tabs = [...(d.tabs ?? []),
                    {id, order: (d.tabs?.length ?? 0) + 1, title}] as DashboardTab[];
            ...
            reset: () => {
                set({dashboard: null, isLoading: false, error: null, isDirty: false, selectedPanelId: null, activeTabId: null});
            },
        }),
        {
            limit: 50,
