...
import {findAvailablePosition, hasOverlap, autoCompact, snapDeltaToGrid, clampGridPosForDrag, clampGridPosForResize} from "../grid-utils";
...
describe("Grid Utils", () => {
  describe("findAvailablePosition", () => {
    ...
    it("should place full-width panel below existing panels", () => {
      const panels = [makePanel("p1", 0, 0, 6, 4), makePanel("p2", 6, 0, 6, 4)];
  ...
  describe("autoCompact", () => {
    it("should move panels up to fill gaps", () => {
      ...
      expect(result.find((p) => p.id === "p2")?.gridPos.y).toBe(4);
    });
    ...
    it("should not move panels if no gaps", () => {
      ...
      const result = autoCompact(panels);
      expect(result.find((p) => p.id === "p1")?.gridPos.y).toBe(0);
      expect(result.find((p) => p.id === "p2")?.gridPos.y).toBe(4);
    });
