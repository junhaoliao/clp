...
describe("mergeDataFrame", () => {
  it("adds a new frame to an empty map", () => {
    ...
    const partial: DataFrame = {
      name: "A",
      fields: [
        {name: "timestamp", type: "time", values: [1000, 2000]},
        {name: "message", type: "string", values: ["hello", "world"]},
      ],
      length: 2,
    };
    ...
    expect(merged.fields[0]!.values).toEqual([1000, 2000]);
    expect(merged.fields[1]!.values).toEqual(["hello", "world"]);
  });
  ...
  it("appends values to an existing frame", () => {
    ...
    const frame2: DataFrame = {
      name: "A",
      fields: [
        {name: "timestamp", type: "time", values: [2000, 3000]},
        {name: "message", type: "string", values: ["second", "third"]},
      ],
      length: 2,
    };
    ...
    expect(merged.fields[0]!.values).toEqual([1000, 2000, 3000]);
    expect(merged.fields[1]!.values).toEqual(["first", "second", "third"]);
  });
