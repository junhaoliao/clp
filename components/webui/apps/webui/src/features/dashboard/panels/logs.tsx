...
export function LogsPanel({data, id, options}: PanelComponentProps) {
  ...
  const handleFilterClick = useCallback((event: {type: string; sourcePanelId: string; payload: Record<string, unknown>}) => {
    if (event.sourcePanelId === id) return;
  ...
  const messageField = frame.fields.find((f) => f.name === "message" || f.name === "msg" || f.name === "log") ?? frame.fields[0];
  const timeField = frame.fields.find((f) => f.type === "time");
  const levelField = frame.fields.find((f) => f.name === "level" || f.name === "severity");
  const showTimestamp = (options["showTimestamp"] as boolean) !== false;
    ...
    <div className="overflow-auto h-full font-mono text-xs leading-relaxed">
      {activeFilter && (
        <div className="flex items-center gap-1 px-1 py-0.5 bg-primary/10 text-primary text-[10px] sticky top-0 z-10">
          <span>Filter: {activeFilter.field} = {String(activeFilter.value)}</span>
          <button
      ...
      {Array.from({length: frame.length}, (_, rowIdx) => {
        ...
        const time = timeField ? timeField.values[rowIdx] : undefined;
        const level = levelField ? String(levelField.values[rowIdx] ?? "") : undefined;

        return (
          <div key={rowIdx} className="flex gap-2 px-1 hover:bg-muted/30 border-b border-border/30">
            {showTimestamp && time !== undefined && (
              <span className="text-muted-foreground shrink-0">
                {typeof time === "number" ? new Date(time).toLocaleTimeString() : String(time)}
              </span>
            )}
            {level && (
            ...
            <span className={wrapLines ? "whitespace-pre-wrap break-all" : "whitespace-nowrap"}>
              {message}
            </span>
          </div>
        );
...
function levelColor(level: string): string {
  ...
  if (l === "error" || l === "err" || l === "fatal" || l === "critical") return "text-red-500";
  if (l === "warn" || l === "warning") return "text-yellow-500";
  if (l === "info" || l === "information") return "text-blue-500";
